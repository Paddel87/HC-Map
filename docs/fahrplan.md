...
- **Stand vom:** 2026-05-01 (laufende Session — sechster Sub-Step **M10.6 (Backup-Service `pg_dump | age | rclone` + Cron-Container)** [ERLEDIGT]. Test-Stand: Roundtrip-Verifikation gegen zwei Postgres-Containern (PostGIS 16-3.5) im eigenen Bridge-Netz mit rclone-Remote `local:` → Schema-Diff = 0 Zeilen, Daten-Diff = 0 Zeilen, PostGIS-Extensions/Indizes/GEOGRAPHY-Werte erhalten. Backend pytest unverändert **246/246** (kein Code-Touch im Backend in M10.6), Frontend unverändert **278/278**. `docker compose -f compose.prod.yml config` und beide Reverse-Proxy-Overlays grün. **Commit-Stand:** noch offen — wird im Anschluss erstellt (Branch `claude/confident-kirch-15d042`).)
- **Laufende Phase:** Phase 1 (MVP) — gestartet
- **Nächste Session — aktiver Schritt:** **M10.7 (GitHub Actions CI + GHCR-Push, Multi-Arch) [OFFEN]**. Plan gemäß ADR-051 §E + §I:
  - `.github/workflows/ci.yml` mit drei Jobs: `backend-lint-test` (Python 3.12, uv, ruff/mypy/pytest mit Postgres-Service-Container), `frontend-lint-test` (Node 22, pnpm, typecheck/lint/format/test), `build-push` (nur `main` + Tags `v*.*.*`, QEMU + buildx, `linux/amd64,linux/arm64`, push zu `ghcr.io/paddel87/hc-map-{backend,frontend,backup}` mit Tag-Schema `:rc`/`:v0.1.0-rc.1`/`:main`/`:sha-*`).
  - `.github/workflows/release.yml` (triggered auf `v*.*.*`-Tags, GitHub-Release mit Notes-Auto-Extract aus CHANGELOG).
  - GHCR-Sichtbarkeit public, `packages: write`-Scope am Workflow.
  - **Akzeptanz:** CI grün auf einem Branch-PR (act-Lokaltest oder echter PR), GHCR-Image-Tags nach Tag-Push prüfbar, Backup-Image ebenfalls multi-arch verfügbar (M10.6 hat das Image vorbereitet — M10.7 publiziert es).
  - **Hinweis:** sobald GHCR den `hc-map-backup`-Tag hat, in `docker/compose.prod.yml` `build:` durch `image: ghcr.io/paddel87/hc-map-backup:${HCMAP_IMAGE_TAG:-rc}` + `pull_policy: always` ersetzen (analog backend/frontend).
...
- **Übersicht über die noch offenen M10-Sub-Steps:** M10.7 (GitHub Actions CI + GHCR), M10.8 (Runbook + README-Restruktur), M10.9 (RC-Smoke + Tag `v0.1.0-rc.1` + GitHub-Pre-Release).

...
- **Stand vom:** 2026-05-01 (laufende Session — siebter Sub-Step **M10.7 (GitHub Actions CI + GHCR-Push, Multi-Arch)** [ERLEDIGT] nach zwei Followup-Fixes durch echte CI-Runs. Test-Stand: Backend pytest **246/246**, Frontend vitest **278/278**, `format:check` clean, `actionlint` clean. CI-Run #2 `gh run 25225805748`: Backend + Backup-Image erfolgreich nach GHCR gepusht (multi-arch `linux/amd64,linux/arm64`); Frontend-Image scheiterte an fehlendem `frontend/public/`-Dir, mit Followup-#2 behoben. CI-Run #3 prüft den finalen Stand. **Commit-Stand:** Branch `claude/confident-kirch-15d042` enthält M10.6, M10.7, prettier-Sweep + `frontend/public/.gitkeep`-Fix.)
...
- **Hinweise zur Repo-Konfiguration (manuelle Schritte des Repo-Eigentümers, nicht Code):**
  - Workflow-Permissions: Default „Read and write" auf dem Repo war bereits ausreichend (Run #2 hat `hc-map-backend` und `hc-map-backup` ohne Settings-Anpassung gepusht). Falls die Defaults restriktiver gesetzt sind: `Settings → Actions → General → Workflow permissions` auf „Read and write".

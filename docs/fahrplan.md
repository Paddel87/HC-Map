...
- **Stand vom:** 2026-05-01 (Sessionende M8.4 — drei Commits 786ab93/609918a/728650e auf main gepusht; M8.5 ist offen.)
- **Laufende Phase:** Phase 1 (MVP) — gestartet
- **Aktiver Schritt:** **M8.5 (Frontend Personen-Verwaltung + Export-UI) [OFFEN].** Nächste konkrete Aktion: `(admin-only)/persons/page.tsx` mit Filter `origin=on_the_fly` + `linkable=true` + `unlinked=true`, Merge-Wizard (Source/Target-Picker → Konflikt-Vorschau → POST `/api/admin/persons/{id}/merge`), Anonymisierungs-Confirm-Dialog (POST `/api/persons/{id}/anonymize`, bestehender Endpoint aus M2), Export-Button (öffnet `adminExportUrl()`-Download). Backend-API ist seit M8.3 vollständig vorhanden — reines Frontend-Sub-Step. Folge-Sub-Step M8.5b/c falls die Aufgabe sich unterteilt.

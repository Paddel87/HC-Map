...
- **Stand vom:** 2026-05-01 (laufende Session — siebter Sub-Step **M10.7 (GitHub Actions CI + GHCR-Push, Multi-Arch)** [ERLEDIGT]. Test-Stand: Backend-CI lokal gegen Postgres-16-3.5-Service-Container repliziert → ruff/format/mypy clean, pytest **246/246 grün** (89 s). Frontend-CI lokal mit `corepack pnpm install --frozen-lockfile` repliziert → typecheck/lint clean, vitest **278/278 grün** (5 s); `format:check` lokal nicht ausgeführt (Node-24-Drift, CI fährt Node 22). `actionlint` (rhysd/actionlint via Docker) auf beiden Workflow-Files clean. CHANGELOG-Extraktion-Awk gegen aktuellen Stand verifiziert. **Commit-Stand:** noch offen — wird im Anschluss erstellt (Branch `claude/confident-kirch-15d042`).)
- **Laufende Phase:** Phase 1 (MVP) — gestartet
- **Nächste Session — aktiver Schritt:** **M10.8 (`ops/runbook.md` + README-Restruktur, Operator-Quickstart) [OFFEN]**. Plan gemäß ADR-051 §H + §I:
  - Neues Dokument `ops/runbook.md` mit VPS-Anforderungen (CPU/RAM/Disk-Untergrenze, Distribution, Domain-/DNS-Voraussetzungen), SSH-Hardening (Key-only, fail2ban, Firewall), Docker-Install (24+), Stack-Start je nach Reverse-Proxy-Wahl (Caddy- vs. Traefik-Pfad), age-Schlüsselgenerierung + sichere Verwahrung (Passwort-Manager + 2-Personen-Backup-Schlüssel als Option), rclone-Setup mit drei Anbieter-Walks (Hetzner Storage Box / Backblaze B2 / generisches S3), Bootstrap-Prozess (`HCMAP_BOOTSTRAP_*` Env), Update-Pfad (`docker compose pull` + restart), Restore-Drill (mit Hinweis: Ziel-DB aus `template0`).
  - README-Restruktur: Operator-Quickstart als oberster inhaltlicher Abschnitt, dann Konfiguration, Backups, Update-Pfad, dann Development-Setup nach unten verschoben, dann Architektur-Verweis und Lizenz. CI-Status-Badge (greift erst nach erstem M10.7-Run).
  - Akzeptanz: Patrick liest Doku durch und schätzt: „Reicht das, um eine fremde HC-Map ans Laufen zu bringen?". Lücken werden im selben Schritt nachgezogen.
...
- **Sub-Folgearbeit aus M10.7:** Sobald der erste `build-push`-Run die GHCR-Pakete `hc-map-{backend,frontend,backup}` befüllt hat und Patrick die Sichtbarkeit auf public umgestellt hat, in `docker/compose.prod.yml` den `backup`-Service von `build:` auf `image: ghcr.io/paddel87/hc-map-backup:${HCMAP_IMAGE_TAG:-rc}` + `pull_policy: always` umstellen (analog backend/frontend). Sub-Schritt ist sehr klein, gehört aber nach erstem CI-Erfolg, nicht in M10.7 vorab.
- **Übersicht über die noch offenen M10-Sub-Steps:** M10.8 (Runbook + README-Restruktur), M10.9 (RC-Smoke + Tag `v0.1.0-rc.1` + GitHub-Pre-Release).
...
- **Initialer CI-Run (`gh run 25225432180`) hat einen Bug aufgedeckt** und wurde mit Folge-Commit korrigiert: Frontend-`format:check` schlug auf Node 22 mit 47 Files an. Die ursprüngliche M10.7-Verifikation hatte `format:check` lokal nicht ausgeführt (Annahme „Node-24-Drift aus M8.5; CI fährt Node 22, dort grün") — die Annahme war falsch. Tatsächliche Ursache: die 47 Files waren seit M5b/M7 nicht mehr durch `prettier --write` gelaufen und in einem nicht-kanonischen Wrap-Zustand committed; das war im Repo unsichtbar geblieben, weil es vor M10.7 keine CI-Stufe für `format:check` gab. **Fix:** `corepack pnpm format` über alle 47 Files; Diff ist rein Whitespace/Wrap (Zeilen-Joining), keine Logik-Änderungen. Cross-Verifikation in einem `node:22-bookworm-slim`-Container (entspricht dem GitHub-Runner): `format:check` clean. Damit ist die Pre-M10.7-Annahme zur Node-Version-Drift hinfällig — nach `prettier --write` ist der Output stabil über Node 22 + 24. Commit als M10.7-Followup, CI-Run #2 prüft.
- **Folge-Anpassung in M10.6 nach M10.7-Image-Publish:** `docker/compose.prod.yml` `backup`-Service umstellen von `build:` auf `image: ghcr.io/paddel87/hc-map-backup:${HCMAP_IMAGE_TAG:-rc}` + `pull_policy: always` analog backend/frontend. Bewusst nicht Teil von M10.7 — erst nach erstem GHCR-Push prüfbar (eigener Sub-Schritt im Anschluss an den ersten erfolgreichen `build-push`-Run).
- **Hinweise zur Repo-Konfiguration (manuelle Schritte des Repo-Eigentümers, nicht Code):**
  - `Settings → Actions → General → Workflow permissions` muss „Read and write" haben, damit `release.yml` (`gh release create`) und `build-push` (Push zu GHCR) den `GITHUB_TOKEN`-Scope `contents: write`/`packages: write` tatsächlich einsetzen können.
  - GHCR-Paket-Sichtbarkeit (`hc-map-backend`, `hc-map-frontend`, `hc-map-backup`) ist nach erstem Push initial **private**; Patrick muss in der GitHub-Package-UI auf „Public" umstellen (passt zu AGPLv3-Linie aus ADR-051 §E).

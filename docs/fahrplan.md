...
- **Stand vom:** 2026-05-01 (M10.3 erledigt — `LICENSE`-File mit AGPLv3-Volltext, SPDX-Identifier in `pyproject.toml`/`package.json`, README-Lizenz-Badge auf `AGPL-3.0-only` umgestellt, Lizenz-Abschnitt mit Multi-Instanz-Hinweis. Compliance-Check: 76 Backend-Pakete + Frontend-Prod-Tree ohne GPL-/proprietäre Treffer; LGPL-Treffer sind über Dynamic-Linking AGPL-kompatibel. Commit-Stand: `origin/main` auf `409619c`, lokal `claude/tender-kilby-8a3576` zwei Commits voraus (`97cd3d0` ADR-051, `c51a397` M10.2), dritter Commit mit M10.3 folgt.)
- **Laufende Phase:** Phase 1 (MVP) — gestartet
- **Aktiver Schritt:** **M10.4 (Einwilligungs-Vorlage `docs/templates/consent-de.md`) [OFFEN]** — als nächster Sub-Step von M10. Strukturierte Markdown-Vorlage mit Platzhaltern (`[GRUPPENNAME]`, `[INSTANZ-URL]`, `[HOSTING-PROVIDER]`, …); inhaltlich abgedeckt: Vertrauensmodell (ADR-001), Anonymisierungs-Kompromiss (ADR-002), on-the-fly-Personenanlage (ADR-014), Aggregat-Statistik (ADR-015), IndexedDB unverschlüsselt (ADR-032), Foto-/Medien-Speicherung (Phase-2-Platzhalter), Widerrufs-/Auskunftsrechte. Verweis aus README + project-context.md §6 + architecture.md. **Sub-Folgearbeit aus ADR-050:** M5c-NACH bleibt [OFFEN], nicht-blockierend. **Nächster Schritt nach M10.4:** M10.5 (`compose.prod.yml` + Caddy/Traefik-Overlays + Prod-ENV-Schema).

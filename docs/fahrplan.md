...
- **Stand vom:** 2026-05-01 (laufende Session — fünfter Sub-Step **M10.5 (`compose.prod.yml` + Reverse-Proxy-Overlays + Prod-ENV-Schema + Migrations-Auto-Run)** [ERLEDIGT]. Test-Stand: Backend pytest **246/246** grün (+15 in `tests/test_migrations_runner.py`: DSN-Helpers, `_should_run`-Gating, Lock-ID-Stabilität, Skip-Pfade ohne DB, Lock-Roundtrip mit echtem Postgres + Concurrent-Wait-Szenario), `ruff check` + `ruff format --check` + `mypy --strict app` clean. Frontend nicht touched, Stand 278/278 aus M10.4 gilt. **Compose-Validierung:** `docker compose -f compose.prod.yml -f compose.caddy.yml config` und Traefik-Variante beide grün. **Commit-Stand:** noch offen — wird im Anschluss erstellt (Branch `claude/inspiring-vaughan-0ead7a`).)
- **Laufende Phase:** Phase 1 (MVP) — gestartet
- **Nächste Session — aktiver Schritt:** **M10.6 (Backup-Service `pg_dump | age | rclone` + Cron-Container) [OFFEN]**. Plan gemäß ADR-051 §D + §I:
  ...
  - Cron-Schedule daily 03:17 UTC + weekly Sunday 03:33 + monthly 1. 03:47 UTC; Retention 14d/56d/365d via `rclone delete --min-age` als zweiter Cron-Job.
  - Restore-Skript: Private-Key + rclone-Pull + age-Decrypt + pg_restore in zweite leere DB.
  - `compose.prod.yml` um `backup`-Service erweitern (greift bereits-reservierte `HCMAP_BACKUP_*`-Env aus M10.5).
  - **Akzeptanz:** Roundtrip-Test (lokal mit `rclone` Remote `local:`): Dump → age → rclone copy → age decrypt → pg_restore → Smoke gegen Restore-DB grün.
...
- **Übersicht über die noch offenen M10-Sub-Steps:** M10.6 (Backup-Service), M10.7 (GitHub Actions CI + GHCR), M10.8 (Runbook + README-Restruktur), M10.9 (RC-Smoke + Tag `v0.1.0-rc.1` + GitHub-Pre-Release).

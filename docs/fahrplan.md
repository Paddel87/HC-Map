...
- **Stand vom:** 2026-05-01 (Sessionende — vier Sub-Steps von M10 in dieser Session erledigt: M10.1 (ADR-051 Strategie-Freigabe), M10.2 (Mail-Backend `aiosmtplib` + Frontend Reset-Pages, Browser-Smoke voll grün), M10.3 (AGPLv3 LICENSE + SPDX + README-Lizenz-Block + Compliance-Check), M10.4 (Einwilligungs-Vorlage `docs/templates/consent-de.md`). Test-Stand am Sessionende: Backend pytest **231/231**, Frontend vitest **278/278**, ruff/mypy/typecheck/lint clean. **Commit-Stand:** vier Commits lokal auf `claude/tender-kilby-8a3576` (`97cd3d0` ADR-051, `c51a397` M10.2, `bf0bb9f` M10.3, `99cf7e8` M10.4); `origin/main` weiter auf `409619c`. Push auf `main` ist Patrick-Aktion und steht aus.)
- **Laufende Phase:** Phase 1 (MVP) — gestartet
- **Nächste Session — aktiver Schritt:** **M10.5 (`compose.prod.yml` + Caddy/Traefik-Overlays + Prod-ENV-Schema) [OFFEN]**. Konkreter Plan gemäß ADR-051 §B + §I:
  - `docker/compose.prod.yml` ohne Reverse-Proxy-Service; App-Services (`db`, `backend`, `frontend`, später `backup`) exposeen ausschließlich interne Container-Ports (kein Host-Port-Mapping außer ggf. Loopback für DB-Backup-Schnittstelle).
  - `docker/compose.caddy.yml` + `docker/Caddyfile.example` — Caddy v2 mit Auto-TLS via Let's Encrypt, ACME-Email aus `HCMAP_ACME_EMAIL`, `reverse_proxy /api/* backend:8000`, sonst `frontend:3000`.
  - `docker/compose.traefik.yml` + `docker/traefik/traefik.yml.example` + `docker/traefik/dynamic.yml.example` — Traefik v3 mit Auto-TLS via Let's Encrypt, Labels-/Dynamic-Config-Mix, ACME-Email analog.
  - Beide Overlays setzen einheitlich `X-Forwarded-Proto`, `X-Forwarded-Host`, `X-Forwarded-For` Header.
  - `.env.example` um Prod-Block erweitert: `HCMAP_DOMAIN`, `HCMAP_BASE_URL` (überschreibt M10.2-Default), `HCMAP_ACME_EMAIL`, `HCMAP_COOKIE_DOMAIN`, `HCMAP_BACKUP_*` (Vor-Reservierung für M10.6).
  - `app/main.py`: Migrations-Auto-Run beim Backend-Start, abgesichert mit Postgres-Advisory-Lock (z. B. `pg_try_advisory_lock(0xHCMAPMIG)`) gegen konkurrierende Backend-Instanzen; Override via `HCMAP_SKIP_MIGRATIONS=1` für Notfälle. Bei Migrations-Fehler `sys.exit(1)`, Compose-Restart-Loop bringt das Problem an die Oberfläche.
  - Backend-uvicorn-Start mit `--proxy-headers --forwarded-allow-ips=*` (vertraut allen Container-Netz-IPs, akzeptabel hinter Reverse-Proxy).
  - **Verifikation:** lokaler Voll-Stack-Test mit beiden Overlays alternativ (Caddy + selbstsigniertes Cert oder Let's-Encrypt-Staging via `localhost`-Workaround; Traefik analog). Bootstrap, Login-Flow, CSRF und Cookie-Secure aktiv. Migrations laufen einmal beim ersten Backend-Start, bei zweitem parallelen Backend-Start kein Re-Run.
...
- **Übersicht über die noch offenen M10-Sub-Steps:** M10.5 (Compose+Reverse-Proxies), M10.6 (Backup-Service `pg_dump | age | rclone`), M10.7 (GitHub Actions CI + GHCR), M10.8 (Runbook + README-Restruktur), M10.9 (RC-Smoke + Tag `v0.1.0-rc.1` + GitHub-Pre-Release).

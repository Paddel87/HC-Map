...
- **Stand vom:** 2026-05-01 (M10.2 erledigt — Mail-Backend `SMTPMailer` (aiosmtplib) + Frontend `/forgot-password` + `/reset-password` produktiv. **Verifikation:** Backend pytest 231/231, Frontend vitest 278/278, ruff/mypy/typecheck/lint clean, voller Browser-Smoke-Roundtrip lokal: forgot → 202 → Reset-URL aus Backend-Log → reset → 200 → Re-Login mit neuem Passwort → 204. Commit-Stand: `origin/main` weiter auf `409619c`; lokal `claude/tender-kilby-8a3576` ein Commit voraus (`97cd3d0` ADR-051), zweiter Commit mit M10.2 folgt.)
- **Laufende Phase:** Phase 1 (MVP) — gestartet
- **Aktiver Schritt:** **M10.3 (LICENSE + Lizenz-Metadaten + README-Header) [OFFEN]** — als nächster Sub-Step von M10. `LICENSE`-File mit AGPLv3-Volltext anlegen, `pyproject.toml::license="AGPL-3.0-only"`, `package.json::"license":"AGPL-3.0-only"`, README-Lizenz-Badge + Hinweis-Block. Compliance-Check: `pip-licenses` und `pnpm licenses list` ad-hoc laufen lassen, dürfen außer AGPL keine GPL-/proprietären Treffer zeigen. **Sub-Folgearbeit aus ADR-050:** M5c-NACH bleibt [OFFEN], nicht-blockierend. **Nächster Schritt nach M10.3:** M10.4 (Einwilligungs-Vorlage `docs/templates/consent-de.md`).

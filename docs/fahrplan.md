...
- **Nächster Schritt:** **M11 (Go-Live Pfad A: Promote RC → `v0.1.0`) [OFFEN]** — Patrick provisioniert seinen eigenen VPS gemäß [ops/runbook.md](../ops/runbook.md), pullt `:0.1.0-rc.1`, fährt den Stack hoch, lädt Mitglieder ein, Bestand wird via M5c (Nachträgliche Erfassung) eingepflegt. Nach mind. 7 Tagen stabilem Betrieb: Git-Tag `v0.1.0` (Final), GHCR-Image-Tags `:0.1.0` + `:0.1` + `:0` + `:latest` werden gesetzt. **M11-Hotfix `M11-HOTFIX-001`** (Issue [#15](https://github.com/Paddel87/HC-Map/issues/15) — Frontend-SSR-Backend-URL): **[ERLEDIGT] 2026-05-02** mit Commit [80ce568](https://github.com/Paddel87/HC-Map/commit/80ce568) auf `main`, ADR-053 (Accepted, Empfehlung A); RC-1-Operator muss bis RC-2 das Compose-File aus `main` ziehen oder die Zeile händisch ergänzen. **M11-Hotfix `M11-HOTFIX-002`** (Issue [#16](https://github.com/Paddel87/HC-Map/issues/16) — Frontend-Image-Healthcheck akzeptiert nur 200): **[ERLEDIGT] 2026-05-02** mit Commit [b781961](https://github.com/Paddel87/HC-Map/commit/b781961) auf `main` (Edit `docker/frontend.Dockerfile:60` `=== 200` → `>= 200 && < 400`, ohne ADR), Issue geschlossen. **M11-Hotfix `M11-HOTFIX-003`** (Issue [#21](https://github.com/Paddel87/HC-Map/issues/21) — strukturierter Access-Logger Variante B): **[ERLEDIGT] 2026-05-02** mit Commit [e2d3021](https://github.com/Paddel87/HC-Map/commit/e2d3021) auf `main`, ADR-054 (`Accepted`, Variante B), 10 neue Tests, Suite 256/256 grün, Issue geschlossen. **M11-Hotfix `M11-HOTFIX-004`** (Issue [#18](https://github.com/Paddel87/HC-Map/issues/18) — Profil-Passwort-Änderungs-Form): **[ERLEDIGT] 2026-05-02** mit Commit [6fe428d](https://github.com/Paddel87/HC-Map/commit/6fe428d) auf `main` (neue `components/profile/password-form.tsx`, `useChangePassword`-Hook in `lib/auth.ts`, Profil-Page-Integration, 4 neue Tests, Suite 282/282 grün, Browser-Verifikation grün, End-to-End-Login-Roundtrip mit altem→neuem Passwort verifiziert), Issue geschlossen. **M11-Hotfix `M11-HOTFIX-005`** (Issue [#19](https://github.com/Paddel87/HC-Map/issues/19) — Defense-in-Depth `BACKEND_INTERNAL_URL` als Image-ENV-Default): **[ERLEDIGT] 2026-05-02** mit Commit [10c8fd1](https://github.com/Paddel87/HC-Map/commit/10c8fd1) auf `main` (Frontend-Dockerfile-`ENV`-Block ergänzt; `docker exec env`-Verifikation grün, Compose-Override-Vorrang verifiziert; lokale Repro von #19 ergab grünes Catalog-UI, Bug ist Production-Compose-spezifisch). HOTFIX-005 hat Hypothese 1 gehärtet, traf den eigentlichen Bug aber nicht. **M11-Hotfix `M11-HOTFIX-006`** (Issue [#19](https://github.com/Paddel87/HC-Map/issues/19) — SQLAdmin auf `/sqladmin/` umziehen, ADR-055): **[ERLEDIGT] 2026-05-03** mit Commit [e2b6e96](https://github.com/Paddel87/HC-Map/commit/e2b6e96) auf `main` und Tag [`v0.1.0-rc.3`](https://github.com/Paddel87/HC-Map/releases/tag/v0.1.0-rc.3) (`:rc` re-mapped auf neue Image-Digests). **Operator-Verifikation auf Nodica1 grün** (Patrick 2026-05-03): nach RC-3-Pull + Traefik-`dynamic.yml`-Patch (`/admin` → `/sqladmin`, Variante 4.3 außerhalb der Compose-Overlays) liefert `GET /admin/catalogs` korrekt `307 → /login?next=…` (Frontend-Auth-Guard), `GET /sqladmin/` `302 → /login` (SQLAdmin-Auth), Catalog-UI im Browser erreichbar. Issue [#19](https://github.com/Paddel87/HC-Map/issues/19) geschlossen. Folge-Doku: Runbook §4.3 um Migrations-Stolperer ergänzt (analog #15-Block). **M11-Hotfix `M11-HOTFIX-007`** (Issue [#22](https://github.com/Paddel87/HC-Map/issues/22) — MapLibre `GeolocateControl` in Karten-Komponenten): **[ERLEDIGT] 2026-05-03** auf Branch `claude/sweet-swirles-73535b` (Frontend-only, drei Files: [`map-view.tsx`](../frontend/src/components/map/map-view.tsx), [`location-picker-map.tsx`](../frontend/src/components/map/location-picker-map.tsx), [`tests/map-view.test.tsx`](../frontend/tests/map-view.test.tsx); kein neuer Stack-Bestandteil — `react-map-gl/maplibre` schon im Tree per ADR-027/041; Suite **282/282** grün, Browser-Verifikation auf `/map` und `/events/new` zeigt `.maplibregl-ctrl-geolocate`-Button neben Zoom-Controls; im `LocationPickerMap` setzt `onGeolocate`-Hook den Marker direkt — kein zweiter Tap). Operator-Wunsch aus dem RC-3-Triage-Dialog 2026-05-03.
...
- **Sub-Folgearbeit aus ADR-050:** **M5c-NACH (Legacy-External-Ref im Edit/Backfill-UI) [ERLEDIGT] 2026-05-04** auf Branch `claude/silly-curran-44aa99` (Frontend-only, drei Komponenten + drei Test-Files; Suite **289/289** grün, +7 neue Cases; Browser-Smoke gegen Voll-Stack zeigt End-to-End-Pfad Backfill → Detail → Edit → Detail → Clear → Detail; RxDB-LWW-Sync ans Backend bestätigt). Das Final-Tag-Followup aus ADR-050 ist damit abgeschlossen.
